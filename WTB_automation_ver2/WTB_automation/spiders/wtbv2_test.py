...
def runfunction():
    ...
    class WTBSpider(scrapy.Spider):
        ...
        def parse(self, response):
            ...
            retailer = response.xpath("//tr[starts-with(@class , 'ciq-record-row')]")
            #//tr[starts-with(@class , 'ciq-record-row')]
            # items['retailers'] = retailer

            # update the list of retailer(Pandas)
            for all in retailer:
                i = all.xpath(".//td[2]/div/text()").extract_first()
            ...
            for ret in retailer:
                s = ret.xpath(".//td[2]/div/text()").extract_first()
                row = response.url
                column = s.strip()
                df.loc[row, column] = ret.xpath(".//a[@class='ciq-buy-now-button']/@href").extract_first()
                yield {
                    'ret' : s,
                    'link' : ret.xpath(".//a[@class='ciq-buy-now-button']/@href").extract_first()
                }
